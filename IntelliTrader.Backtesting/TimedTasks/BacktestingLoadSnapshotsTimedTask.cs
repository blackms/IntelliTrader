            ...
            lock (backtestingService.SyncRoot)
            {
                ...
                if (allSignalSnapshotPaths.TryDequeue(out string currentSignalsSnapshotPath))
                {
                    try
                    {
                        ...
                        IEnumerable<ISignal> data = MessagePackSerializer.Deserialize<IEnumerable<SignalData>>(currentSignalsSnapshotBytes).Select(s => s.ToSignal()).ToList();
                ...
                if (allTickerSnapshotPaths.TryDequeue(out string currentTickersSnapshotPath))
                {
                    try
                    {
                        ...
                        IEnumerable<ITicker> data = MessagePackSerializer.Deserialize<IEnumerable<TickerData>>(currentTickersSnapshotBytes).Select(t => t.ToTicker()).ToList();
